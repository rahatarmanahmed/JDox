...
public class WebViewActivity extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		} else
		{
			...
			String path = this.getIntent().getStringExtra(
					MainActivity.EXTRA_INTERNAL_PATH);
			String jdocPath = this.getIntent().getStringExtra(
					MainActivity.EXTRA_JDOC_PATH);
			try
			{
		...
		if (jdoc != null) // load archive
		{
			...
			Thread thread = new Thread() {
				...
				public void run()
				{
					try
					{
						InputStream in = jdoc.getInputStream(jdoc
								.getEntry(fUrl));
